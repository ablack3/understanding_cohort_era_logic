DROP TABLE IF EXISTS temp.tmp_de;
...
(
    SELECT row_number, person_id, drug_concept_id, drug_sub_exposure_start_date, drug_sub_exposure_end_date, drug_exposure_count
        , JULIANDAY(drug_sub_exposure_end_date, 'unixepoch') - JULIANDAY(drug_sub_exposure_start_date, 'unixepoch') AS days_exposed
    FROM cteSubExposures
)
--------------------------------------------------------------------------------------------------------------
, cteEndDates (person_id, ingredient_concept_id, end_date) AS -- the magic
(
    SELECT person_id, ingredient_concept_id, CAST(STRFTIME('%s', DATETIME(event_date, 'unixepoch', (-30)||' days')) AS REAL) AS end_date -- unpad the end date
    FROM
    (
        SELECT person_id, ingredient_concept_id, event_date, event_type,
        MAX(start_ordinal) OVER (PARTITION BY person_id, ingredient_concept_id
        ...
        FROM (
            ...
            SELECT person_id, ingredient_concept_id, CAST(STRFTIME('%s', DATETIME(drug_sub_exposure_end_date, 'unixepoch', (30)||' days')) AS REAL), 1 AS event_type, NULL
            FROM cteFinalTarget
        ) RAWDATA
    ) e
    WHERE (2 * e.start_ordinal) - e.overall_ord = 0
...
row_number()over(order by person_id) drug_era_id
    ...
    , JULIANDAY(drug_era_end_date, 'unixepoch') - JULIANDAY(MIN(drug_sub_exposure_start_date), 'unixepoch')-SUM(days_exposed) as gap_days
FROM
cteDrugEraEnds dee
